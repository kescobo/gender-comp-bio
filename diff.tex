...
%DIF DEL manuscript_initial.tex   Thu Apr 13 11:15:17 2017
%DIF ADD manuscript.tex           Thu Apr 13 11:15:18 2017
...
\DIFdel{). }\DIFdelend \DIFaddbegin \DIFadd{$P_{female} = 0.376 \pm 0.003$ }\DIFaddend (Fig 1A, Table 1). We observed the same trend in papers labeled with the computational biology (comp) MeSH term, though the $P_{female}$ at every author position was 4-6  percentage points lower. An analysis of publications by year suggests that the gender gaps in both biology and computational biology are narrowing, but by less than 1 percentage point  per year (for bio, change in \DIFdelbegin \DIFdel{$P_{female}$ = 0.0035 / year, 95\% CI = }%DIFDELCMD < [%%%
...
\DIFdelend \DIFaddbegin \DIFadd{$P_{female} = 0.0049 \pm 0.0008 / year$}\DIFaddend ). However, the discrepancy between biology and computational biology has been consistent over time (Fig 1B).

\begin{figure}[!h]
  \caption{
  A: Mean probability that an author in a given position is female for primary \DIFdelbeginFL \DIFdelFL{article }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{articles }\DIFaddendFL indexed in Pubmed with the MeSH term Biology (black) or Computational Biology (grey). The bio dataset is inclusive of papers in the comp dataset. Error bars represent 95\% confidence intervals. B: Mean probability that an author \DIFaddbeginFL \DIFaddFL{in a given position }\DIFaddendFL is female for \DIFaddbeginFL \DIFaddFL{primary articles indexed in Pubmed with the MeSH term Biology (black) or Computational Biology (grey). The bio dataset is inclusive of papers in the comp dataset. Error bars represent 95\% confidence intervals. C: Mean probability that an author is female for }\DIFaddendFL publications in a given year. Error bars represent 95\% confidence intervals. \DIFdelbeginFL \DIFdelFL{C}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{D}\DIFaddendFL : Mean probability that the first (F), second (S), penultimate (P) or other (O) author is female for publications where the last author is male (\DIFdelbeginFL \DIFdelFL{$P_{female} < 0.2$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{female}$ < 0.2}\DIFaddendFL ) or female (\DIFdelbeginFL \DIFdelFL{$P_{female} > 0.8$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{female}$ > 0.8}\DIFaddendFL ). Papers where the gender of the last author was uncertain or could not be determined were excluded. Error bars represent 95\% confidence intervals.}
  \label{fig1}
\end{figure}
...
\begin{figure}[!h]
  \caption{
  A: Mean probability that an author is female for every journal that had at least 1000 authors in our dataset. Grey bars represent journals that have the words “Bioinformatics,” “Computational,” “Computer,” “System(s),” or “omic(s)” in their title. Vertical line represents the median for female author representation. See also S1 Table. B: Mean probability that an author is female for articles in the “Bio” dataset (black dot) or in the “Comp” dataset (open square) for each journal that had at least 1000 authors plotted against the journals’ 2014 impact factor. Journals that had computational biology articles are included in both datasets. \DIFdelbeginFL \DIFdelFL{Pearson’s correlation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{An ordinary least squares regression }\DIFaddendFL was \DIFdelbeginFL \DIFdelFL{calculated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{performed }\DIFaddendFL for each dataset\DIFdelbeginFL \DIFdelFL{independently}\DIFdelendFL . Bio: \DIFdelbeginFL \DIFdelFL{$r = ­0.246$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$m = -0.00264$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$95\% CI = [­0.391, ­0.088]$, $p = 0.0025$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{Z>|z|} = 0.0022$}\DIFaddendFL . Comp: \DIFdelbeginFL \DIFdelFL{$r = ­0.052$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$m = -0.0007893406044121227$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$95\% CI = [­0.4458, 0.3592], p = 0.8106.$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{Z>|z|} = 0.568$.}\DIFaddendFL }
  \label{fig2}
\end{figure}

One possible explanation might be that women are less likely to publish in high-impact journals, so we considered the possibility that the differences in the gender of authors that we observe could be the result of differences in impact factor between papers published in biology versus computational biology publications. We \DIFdelbegin \DIFdel{therefore }\DIFdelend compared the $P_{female}$ of authors in each journal with that journal’s 2014 impact factor (Fig 2B). There is a marginal but significant \DIFdelbegin \DIFdel{(p = 0.0025) negative correlation }\DIFdelend \DIFaddbegin \DIFadd{negative correlation ($-0.00264$, $P_{Z > |z|} = 0.0022$) }\DIFaddend between impact factor and gender for the biology dataset\DIFdelbegin \DIFdel{(r2 = 0.06, 95\% CI = }%DIFDELCMD < [%%%
...
\cite{Ghiasi2015}}%DIFAUXCMD
. It is, however, consistent with a previous studies from mathematics \mbox{%DIFAUXCMD
\cite{Mihaljevic-Brandt2016}}%DIFAUXCMD
...
Women were more likely to be authors in quantitative biology \DIFaddbegin \DIFadd{manuscripts }\DIFaddend than in computer science \DIFaddbegin \DIFadd{manuscripts }\DIFaddend in first, second, and middle author positions (Fig 3A, Table 3)\DIFaddbegin \DIFadd{. We found no significant difference in the frequency of female authors in the last or penultimate author positions in these two datasets}\DIFaddend , though the conventions for determining author order are not necessarily the same in computer science \DIFaddbegin \DIFadd{as in biology}\DIFaddend . Nevertheless, women had higher representation in quantitative biology than in computer science for all years except 2009 (Fig 3B). Interestingly, there is a slight but significant (\DIFdelbegin \DIFdel{p < 0.05}\DIFdelend \DIFaddbegin \DIFadd{$0.0052 / year$, $P_{Z > |z|} < 0.005$}\DIFaddend ) increase in the proportion of female authors over time in quantitative biology, while there’s no significant increase in \DIFaddbegin \DIFadd{female representation in }\DIFaddend computer science preprints.

\begin{figure}[!h]
  \caption{
  A: Mean probability that an author in a given position is female for all preprints in the arXiv quantitative biology (black) or computer science (grey) categories between 2007 and 2014. Error bars represent 95\% confidence intervals. B: Mean probability of authors being female in arXiv preprints in a given year. Error bars represent 95\% confidence intervals. Slopes were determined using \DIFdelbeginFL \DIFdelFL{linear }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ordinary least squares }\DIFaddendFL regression. The slope for q\DIFaddbeginFL \DIFaddFL{\-}\DIFaddendFL bio is slightly positive (\DIFdelbeginFL \DIFdelFL{$p < 0.5$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p < 0.05$}\DIFaddendFL ), but the slope for cs is not.}
  \label{fig3}
\end{figure}
...
This dataset \cite{Bonham2016a} contains all preprints with the label “\DIFdelbegin \DIFdel{q\-bio}\DIFdelend \DIFaddbegin \DIFadd{q-bio}\DIFaddend ” from 2003 (when the section was introduced) to 2014. \DIFdelbegin \DIFdel{Downloaded }\DIFdelend \DIFaddbegin \DIFadd{This set contains 41,637 records and was downloaded }\DIFaddend on 10 June, 2016.
...
\begin{figure}[!h]
  \DIFaddFL{A: Mean probability that an author in a given position is female for primary articles indexed in Pubmed with the MeSH term Biology (black), Computational Biology (gray) or for those articles with Biology }\textit{\DIFaddFL{but not}} \DIFaddFL{Computational biology (white). Error bars represent 95\% confidence intervals. B: Mean probability that an author is female for articles in the “Bio” dataset (black) in the “Comp” dataset (white), or for articles in the Bio }\textit{\DIFaddFL{but not}} \DIFaddFL{Comp (gray) for each journal that had at least 1000 authors plotted against the journals’ 2014 impact factor. Excluding computational publications from the biology dataset does not substantially alter the correlation between impact factor and $P_{female}$.}}
  \label{S2 Fig}
